...
const AuthPage = () => {
  ...
  return (
    <div className="min-h-screen w-full lg:grid lg:grid-cols-2">
      {/* ── Form Panel (left) ── */}
      <main className="flex min-h-screen flex-col items-center justify-center overflow-y-auto px-4 py-8 lg:py-10">
        <div className="w-full max-w-sm">
          ...
          {isSignIn ? (
            <>
              <h1 className="text-[24px] font-black text-[var(--text-heading)]">Sign in</h1>
              <p className="mt-1 text-[13px] text-[var(--text-muted)]">
                New to PlaysGo?{" "}
                <button
                  onClick={toggleMode}
                  className="font-bold text-[var(--brand)] transition hover:text-[var(--brand-hover)] cursor-pointer"
                >
                  Create an account
                </button>
              </p>

              <form onSubmit={handleSignIn} noValidate className="mt-5 space-y-3">
            ...
            <>
              <h1 className="text-[24px] font-black text-[var(--text-heading)]">
                Create your account
              </h1>
              <p className="mt-1 text-[13px] text-[var(--text-muted)]">
                Already a member?{" "}
              ...
              <ul className="mt-7 space-y-3.5">
                {SIGNUP_BENEFITS.map((benefit) => (
                  <li key={benefit} className="flex items-start gap-3 text-[14px] text-white/85">
                    <CheckCircleIcon className="mt-0.5 h-5 w-5 shrink-0 text-white" strokeWidth={2} />
                    {benefit}
                  </li>
                ))}
              </ul>
